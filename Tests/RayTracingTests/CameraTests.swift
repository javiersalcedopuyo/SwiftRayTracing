...
final class CameraTests: XCTestCase
{
    ...
    {
        let cam = Camera(w:128, h:128, aperture: 0.0) // Default position is (0,0,0)

        XCTAssert(cam.lowerLeftCorner.dot(cam.right)   < 0.0, "Lower Left Corner is on the right!")
        XCTAssert(cam.lowerLeftCorner.dot(cam.up)      < 0.0, "Lower Left Corner is over the camera!")
        XCTAssert(cam.lowerLeftCorner.dot(cam.forward) > 0.0, "Lower Left Corner is behind the camera!")
    }

    func testUpdateLowerLeftCorner()
    {
        let cam = Camera(w:128, h:128, aperture: 0.0)
        let llc = cam.lowerLeftCorner

        let translation = Vec3(x:0.0, y:20.0, z:400.0)
        cam.moveTo( translation )

        XCTAssertEqual( cam.lowerLeftCorner, llc + translation )
    }
    ...
    {
        let cam = Camera(w:8, h: 8, aperture: 0.0)
